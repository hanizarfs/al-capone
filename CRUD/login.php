...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['login'])) {
    ...
    $stmt = $mysqli->prepare("SELECT id, username, email, password_hash, status, account_status FROM users WHERE email = ?");
    ...
    if ($result->num_rows === 1) {
        ...
        if (password_verify($password, $user['password_hash'])) {
            if ($user['account_status'] == "Inactive") {
                $_SESSION['error_message'] = 'Your account is currently inactive. Please contact the administrator for assistance.';
